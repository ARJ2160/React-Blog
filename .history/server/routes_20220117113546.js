...
router.put("/postsdata/update/:id", async (req, res, next) => {

    const filter = { _id: req.params.id } //Filter Condition
    const { title, postBody, author, imagesrc } = req.body
    const update = {title, postBody, author, imagesrc}
    Posts.findByIdAndUpdate(filter, { $set: { update } }, { new: true, useFindAndModify: true },
        (err, data) => {
            if (err) {
                console.log(err);
                throw new Error(err)
