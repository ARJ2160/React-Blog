...
const BlogDetails = () => {
    ...
    const handleEdit = () => { 
        fetch('http://localhost:5000/postsdata/update/' + blog._id, { method: 'POST' })
            .then(() => history.push('/postsdata/' + blog._id))
            .catch(err => console.log(err))
    }
        ...
        <article className="blog-main-content" >
          <div className="main-content-header">
            ...
            <div className="content-image">
                <img className="" src={blog.imagesrc} alt="" />
            </div>
          </div>

          <div className="blogdetails-body"><p>{`${blog.postBody}`}</p></div>
          <button className="blogdetails-delete" onClick={handleEdit}>Edit Post</button>
